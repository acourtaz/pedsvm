%Written by Damien Jullié, adapted from poolnorm2, to 900 frames stream,
%exo at 101
     ...
     %sort sheets
        for j=1:size(sheets,2)
           ...
           %average for the ten first green events 
            ...
            figure 
            scaletime=[-100:300]*0.1;
            errorbar(scaletime,moyenne, ET,'-og','MarkerEdgeColor','k')
            xlabel('time')
            ...
            %492 car freq acquisition= 0.122s
            freq=(600*p(1))/dataGreen(1,13);
...
if isempty(finalpers)
    Avfinalpers=ones(1,401);
    ETfinalpers=ones(1,401);
else
...
if isempty(finaltrans)
    Avfinaltrans=ones(1,401);
    ETfinaltrans=ones(1,401);
else
...
figure
errorbar(-100:300,Avfinaltrans, ETfinaltrans,'-ob','MarkerEdgeColor','b')
hold on
errorbar(-100:300,Avfinalpers, ETfinalpers,'-oc','MarkerEdgeColor','c')
...
if isempty(finalpersr)
    Avfinalpersr=ones(1,401);
    ETfinalpersr=ones(1,401);
else  
...
if isempty(finaltransr)
    Avfinaltransr=ones(1,401);
    ETfinaltransr=ones(1,401);
else            
...
figure
errorbar(-100:300,Avfinaltransr, ETfinaltransr,'-ob','MarkerEdgeColor','b')
hold on
errorbar(-100:300,Avfinalpersr, ETfinalpersr,'-oc','MarkerEdgeColor','c')
...
legendframe=(-100:1:dataGreen(3,3))*0.122;
