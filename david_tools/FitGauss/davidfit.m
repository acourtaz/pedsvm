...
if nargin == 0
   ...
   uicontrol('callback','davidfit fullforward','string','Play Forward','position',[125,5,80,20]);
   uicontrol('callback','davidfit fullbackward','string','Play Backward','position',[45,5,80,20]);
   high = double(max(max(max(M))));
   low = double(min(min(min(M))));
   
   uicontrol('style','slider',...
      'callback','davidfit scale',...
      'min',low,'max',high-3,...
      'value',min(min(min(M))),...
   ...
   uicontrol('callback','davidfit goto','string','frame#','style','slider',...
   ...
   uicontrol('String','Select Center','callback','davidfit activate','style','toggle',...
      'position',[45,25,80,20])
   
   set(gcf,'UserData',M,'keypressfcn','davidfit key','doublebuffer','on'...
      ,'colormap',map)
...
if get(gcbo,'value')
   pixvalm off
   mzoom off
   set(findobj('tag','movi'),'buttondownfcn','davidfit gofit')
end
...
options = optimset(optimset('lsqnonlin'),'largescale','off',...
   ...
   'maxiter',150,'tolfun',1,'tolx',5e-3,'jacobian','on');
...
[xl,xld] = uiputfile('*.xls','Save an Excel file output?');
...
if sub_back
   ...
   if from_file
      ...
      small_background = small_average - funct +back_fit(1);
      ...
      [xs,ys] = meshgrid(1:size(large_background,1));
      back_fit(3:4) = back_fit(3:4)+10;
      large_background = -reshape(gauss3dfree(back_fit,xs,ys,large_average),size(large_background))+back_fit(1);
   end
end
...
X0 = [min(min(img)),max(max(img))-min(min(img)),center,center,1,0,0];
