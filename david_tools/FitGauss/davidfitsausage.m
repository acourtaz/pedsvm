function davidfitsausage(action)
if nargin == 0
   ...
   cd(stkd)
   uicontrol('String','Select Center','callback','davidfitsausage activate','style','toggle',...
      'position',[67,15,80,15])
   uicontrol('String','Use Old Background','callback','davidfitsausage gofit',...
   ...
   uicontrol('style','checkbox','position',[5,135,15,15],'tag','sfits','value',1,...
      'callback','davidfitsausage toggle_linescans')
   ...
   uicontrol('string','Stop','callback','davidfitsausage goto','position',[40,30,45,15])
   uicontrol('callback','davidfitsausage fullforward','string',...
      'Play -->','position',[130,30,45,15])
   uicontrol('callback','davidfitsausage fullbackward','string',...
      '<-- Play','position',[85,30,45,15])
   high = double(max(max(max(M(:,:,1:end-1)))));
   low = double(min(min(min(M(:,:,1:end-1)))));
   uicontrol('style','slider',...
      'callback','davidfitsausage scale',...
      'min',low,'max',high-3,...
      'value',low,...
   ...
   uicontrol('callback','davidfitsausage goto','string','frame#','style','slider',...
   ...
   set(gcf,'UserData',M,'keypressfcn','davidfitsausage key','doublebuffer','on'...
      ,'colormap',map)
...
if get(gcbo,'value')
   pixvalm off
   mzoom off
   set(findobj('tag','movi'),'buttondownfcn','davidfitsausage gofit')
end
...
if use_old
   [backfile,backfiled] = uigetfile([xld,'*.mat'],'Where is the old Background');
   ...
   if distract
      ...
      if use_psf
         ...
         back_fit_excel = {'Offset' back_fit(1) 'Amplitude 1' back_fit(2) ...
               'X 1' back_fit(3)+x,'Y 1', back_fit(4),...
      ...
      if use_psf
         ...
         [xs,ys] = meshgrid(-small_dim:small_dim);
         funct = reshape(gauss3dfree(back_fit,xs,ys),size(small_average));
         small_background = -reshape(gauss3dfree(back_fit(1:5),xs,ys,small_average),...
            size(small_average))+back_fit(1);
         [xs,ys] = meshgrid(-large_dim:large_dim);
         large_background = -reshape(gauss3dfree(back_fit(1:5),xs,ys,large_average),...
...
if xl
   ...
   small_saus = mksausage(size(small_backsub),small_dim);
   large_saus = mksausage(size(large_backsub),large_dim);
   sum(sum(small_saus(:,:,1)))
   sum(sum(large_saus(:,:,1)))
   smalltotal = sum(sum(small_backsub.*small_saus));
   largetotal = sum(sum(large_backsub.*large_saus));
...
function sausage = mksausage(movi_size,small_r)

center = floor((movi_size(1)+1)/2);
big_r = center - 1;
[x,y] = meshgrid(-big_r:big_r);
distance = sqrt(x.^2+y.^2);
circle = distance<small_r;
sausage = circle(:,:,ones(1,movi_size(3)));
