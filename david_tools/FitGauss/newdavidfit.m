...
if nargin == 0
   ...
   uicontrol('style','edit','string',int2str(15),'position',[0,100,25,20],...
      'tag','fitRadius','horizontalalignment','left')
   uicontrol('style','text','string','Fit Radius','position',[25,100,55,15])
   uicontrol('style','edit','string',int2str(25),'position',[0,80,25,20],...
      'tag','movieRadius','horizontalalignment','left')
   uicontrol('style','text','string','Movie Radius','position',[25,80,55,15])
   
   %controls inhierited from play
...
function gofit
large_dim = str2num(get(findobj('tag','movieRadius'),'string'));
small_dim = str2num(get(findobj('tag','fitRadius'),'string'));
...
if any(([x,y]-large_dim)<1|([x,y]+large_dim)>[ncols,nrows])
   disp('You Chose a point too close to the edge of the cell')
   return
end
...
if xl
   ...
   tit = {[num2str(size(small_background,1),2),'X',num2str(size(small_background,1),2),...
   ...
   if ~use_old
      ...
      bottomright = get(hsheet,'cells',39,12);
      hrange = get(hsheet,'range',topleft,bottomright);
      hrange.value = {[num2str(size(small_background,1),2),'X',num2str(size(small_background,1),2),...
            ...
            'X-Coord Center of Backsub Movie',x,...
         'Y-Coord Center of Backsub Movie',y}';
...
if sfits&xl
   ...
   adjfits(:,3) = x+fits(:,3);
   adjfits(:,4) = y+fits(:,4);
   tit = {'Frame','Offset','Amplitude','Center x','Center y','Sigma','xslope','yslope','FWHM'};
...
if slinescans&xl
   ...
   scans = zeros(nframes,small_dim+1);
   stdscans = scans;
   for frame = 1:nframes
      waitbar(frame/nframes,h)
   ...
   tit2 = [{'Frame','Radius 0','Radius 1'},num2cell(2:small_dim)];
   [scan_result{1,:}] = deal(tit2{:});
   [stdscan_result{1,:}] = deal(tit2{:});
