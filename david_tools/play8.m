function play8(action)

%Upgrade from play7: adds the possibility to make polygonal ROI for
%quantification or keeping/removing events
...
%%%controls for playing the movie
   uicontrol('string','Stop','callback','play8 goto','position',[10,30,45,15])
   uicontrol('callback','play8 fullforward','string',...
      'Play -->','position',[100,30,45,15])
   uicontrol('callback','play8 fullbackward','string',...
      '<-- Play','position',[55,30,45,15])
   uicontrol('callback','play8 goto','string','frame#','style','slider',...
...
%%%controls for the scale
   ...
   uicontrol('style','slider','callback','play8 scale',...
...
%%%controls for keeping and removing planes
   ...
   uicontrol('string','Ann > Thresh','position',[30,345,70,15],...
       ...
       'tag','AnnTh','callback','play8 annoThr')
...
if ~isempty(events)
   line(events(:,1),events(:,2),'lineStyle','none','marker','o',...
      'markerEdgeColor','r','buttondownfcn','play8 pos')
end
img = M(:,:,frame);
...
% N = 20; %Maximum number of subregions
...
uicontrol('string','Save quantif','position',[10,60,60,15],'tag','svQtf',...
    'callback','play8 svQuant', 'userdata',data)
plot(fluo)
...
for i=1:size(data,2)
    if ~isempty(data{i})
        line(data{i}(:,1),data{i}(:,2),'lineStyle','none','marker','+',...
            'markerEdgeColor','c', 'buttondownfcn','play8 pos')
    end
end
currEv = data{frame};
if ~isempty(currEv)
