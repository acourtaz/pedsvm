...
if nargin == 0
   ...
   uicontrol('string','Stop','callback','play6 goto','position',[10,30,45,15])
   uicontrol('callback','play6 fullforward','string',...
      'Play -->','position',[100,30,45,15])
   uicontrol('callback','play6 fullbackward','string',...
      '<-- Play','position',[55,30,45,15])
   uicontrol('callback','play6 goto','string','frame#','style','slider',...
   ...
   uicontrol('style','slider','callback','play6 scale',...
   ...
   uicontrol('string','Ann > Thresh','position',[30,345,70,15],...
       ...
       'tag','AnnTh','callback','play6 annoThr')
...
if ~isempty(events)
   line(events(:,1),events(:,2),'lineStyle','none','marker','o',...
      'markerEdgeColor','r','buttondownfcn','play6 pos')
end
img = M(:,:,frame);
...
[f,p] = uiputfile([stk(1:c),'_mask.txt'],...
...
for i=1:size(data,2)
    if ~isempty(data{i})
        line(data{i}(:,1),data{i}(:,2),'lineStyle','none','marker','o',...
            'markerEdgeColor','r','buttondownfcn','play6 pos')
    end
end
currEv = data{frame};
