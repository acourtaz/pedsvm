...
if nargin == 0
   ...
   uicontrol('string','Stop','callback','play3 goto','position',[10,30,45,15])
   uicontrol('callback','play3 fullforward','string',...
      'Play -->','position',[100,30,45,15])
   uicontrol('callback','play3 fullbackward','string',...
      '<-- Play','position',[55,30,45,15])
   uicontrol('callback','play3 goto','string','frame#','style','slider',...
   ...
   uicontrol('style','slider','callback','play3 scale',...
   ...
   uicontrol('style','toggle','position',[30,195,30,15],'string','Pick',...
       'tag','pick','callback','play3 pick','value',0)
   uicontrol('string','Show All','position',[60,195,50,15],...
       'callback','play3 showAll')
   uicontrol('string','Load','position',[30,180,40,15],'tag','ld',...
       'callback','play3 ld')
   uicontrol('string','Save','position',[70,180,40,15],'tag','sv',...
       'callback','play3 sv','userdata',data)
   %%%controls for the zoom button
   uicontrol('style','toggle','position',[40,240,60,15],'string','ZOOM',...
       'tag','zoomOn','callback','play3 zoomToggle','value',0)
   %%%controls to select a region (to save a cropped movie/frame)
   uicontrol('style','text','position',[30,300,80,15],'string','REGION',...
       'tag','Region','userdata',[])
   uicontrol('style','toggle','position',[30,285,40,15],'string','Select',...
       'tag','selectR','callback','play3 selReg','value',0)
   uicontrol('string','Show','position',[70,285,40,15],'tag','showR',...
       'callback','play3 shReg')
   uicontrol('string','load','position',[30,270,30,15],'tag','ldR',...
       'callback','play3 ldReg')
   uicontrol('string','save','position',[60,270,30,15],'tag','sdR',...
       'callback','play3 svReg')
   uicontrol('string','erase','position',[90,270,30,15],'tag','erR',...
       'callback','play3 erReg')
   set(gcf,'UserData',M,'keypressfcn','play3 key','doublebuffer','on',...
      'colormap',map)
...
if ~isempty(events)
   line(events(:,1),events(:,2),'lineStyle','none','marker','o',...
      'markerEdgeColor','r','buttondownfcn','play3 pos')
end
img = M(:,:,frame);
...
for i=1:size(data,2)
    if ~isempty(data{i})
        line(data{i}(:,1),data{i}(:,2),'lineStyle','none','marker','+',...
            'markerEdgeColor','c','buttondownfcn','play3 pos')
    end
end
currEv = data{frame};
if ~isempty(currEv)
...
trc = ~isempty(strfind(f,'.trc')); 
%will subtract 1 to coordinates if saved as a .trc file
%WARNING: this file does not have the structure as the MIA generated .trc
%file (no 'event' lines, 4 columns only)
output = [];
k = 0;
