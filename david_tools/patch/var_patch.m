function [avNormMean,varNorMean] = var_patch(pulse)
...
for j=1:numTraces
    ...
    varIntNorm(:,j) = var_int(:,j)./av_int(1,j);
end
avNormMean = mean(avIntNorm,2);
varNorMean = mean(varIntNorm,2);
figure
plot(avNormMean,varNorMean)