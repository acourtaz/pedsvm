...
default = {num2str(550),num2str(20)};
lines = 2;
...
R = strfind(f,'R');
if R
    [fcln,pcln] = uiputfile([f(1:R),num2str(AccFr),f(R+2:end)]...
      ,'Where to put the cleaned up event file');  
else
    [fcln,pcln] = uiputfile([f(1:strfind(f,'cln')+2),num2str(AccFr),f(strfind(f,'cln')+4:end)]...
        ,'Where to put the cleaned up event file');
end
...
nbEv=size(startEv,1);
disp(['There were ',num2str(size(startEvents,1)),' events in the original cln file'])
