...
'Higher limit H of the pixel values used for background (0<= L < H <=1)'};
   ...
   set(handle_rem,'SelectionChangeFcn','browseEvents3 removeEvent')
...
% controls to browse ministacks
   ...
   uicontrol('callback','browseEvents3 previousEvent','string','PREVIOUS',...
   ...
   uicontrol('string','Go','callback','browseEvents3 gotoEvent',...
...
% controls to write comments
   ...
   uicontrol('tag','commentEvent','callback','browseEvents3 addComment',...
       'string','Add','position',[65,105,25,15],'userdata',comments)
   uicontrol('style','edit','tag','comment','position',[5,88,100,17],...
      'horizontalalignment','left')
   uicontrol('callback','browseEvents3 loadComments','string','Load',...
       'position',[20,73,35,15])
   uicontrol('callback','browseEvents3 saveComments','string','Save',...
       'position',[55,73,35,15])
   
% misc. controls   
   Fluo = cell(1,4);
   uicontrol('callback','browseEvents3 writeExcel','string','WriteXLS',...
       'position',[20,55,70,15],'tag','Fluo','userdata',Fluo,...
       'tooltipstring',' Write failed/passed events in summary excel file')
   uicontrol('callback','browseEvents3 removeTRC','string','RemoveTRC',...
       'position',[20,40,70,15],'tag','TRC','userdata',sTRC,...
       'tooltipstring',' Removes tagged events from trc file permanently')
%%%
   
   set(gcf,'UserData',M,'keypressfcn','browseEvents3 keyBrowse','doublebuffer','on',...
      'colormap',map,'tag',[stkd,stk])
      %'closerequestfcn','browseEvents3 saveRemovedandClose')
   if events(1,1) == 0
...
uicontrol('style','slider',...
      'callback','browseEvents3 scaleAv',...
...
uicontrol('style','slider',...
      'callback','browseEvents3 scaleGr',...
...
uicontrol('style','slider',...
      'callback','browseEvents3 scaleRe',...
...
uicontrol('callback','browseEvents3 goto','tag','frame#',...
      'style','slider',...
...
evCrd = ...
['  Event ',num2str(numEv),' Start at X ',num2str(xGreen),', Y ',num2str(yGreen),', fr ',num2str(frame)];
h = title(['Cell ',cellNum(c:c2),evCrd,'  Frame # = ',num2str(-param(2))],...
    'interpreter','none');
set(h,'userdata',['Cell ',cellNum(c:c2),evCrd])
axis image
pixvalm
browseEvents3 scaleGr
...
elseif get(gcf,'currentcharacter') == 'n'
    browseEvents3 nextEvent
elseif get(gcf,'currentcharacter') == 'p'
    browseEvents3 previousEvent
end
