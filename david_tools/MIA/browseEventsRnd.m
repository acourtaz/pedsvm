function browseEventsRnd(action)
 
%Written by DP June 4th 2015; updated 15/01/2016 Adapted from browseEvents3
%The event order is randomized, so the user does not know when in the
%recording the event occurs
%Functions have been added for saving and loading previously generated Rnd
%files (saveRnd, loadRnd)
...
if nargin == 0
   ...
   MR5 = stkread(stkR5,stkdR5);
       pause(0.1)
       %%% Modifs Mo (+ see line 1086: how to put prot name in sLab?)
...
% Makes a randomized numbering of events
if events(1,1) == 0
...
RndEv = randperm(size(startEv,1))';
RndEv = cat(2,RndEv,startEv);
sRndEv = sortrows(RndEv); %The randomized events are coded here
...
'Higher limit H of the pixel values used for background (0<= L < H <=1)'};
   [sizeMini before after av_high rCircle rAnn l_per h_per] = ...
	   numinputdlg(prompt,'Parameters for the ministacks',1,[25 20 20 5 2 5 0.2 0.8]);
   ...
   handle_rem = uibuttongroup('units','pixels','position',[25,125,60,60],...
       'tag','removeCause','backgroundcolor','c');
   ...
   set(handle_rem,'SelectionChangeFcn','browseEventsRnd removeEvent')
...
% controls to browse ministacks
   ...
   uicontrol('callback','browseEventsRnd previousEvent','string','PREVIOUS',...
       'position',[20,300,70,15])
   uicontrol('callback','browseEventsRnd nextEvent','string','NEXT',...
       'position',[20,285,70,15])
   uicontrol('callback','browseEventsRnd nextEvent','string','NEXT',...
       'position',[100,45,70,40],'userdata',sRndEv,'tag','sRnd')
   uicontrol('style','text','string','#','position',[20,270,10,15],...
       'tag','coeff','userdata',coeff)
   uicontrol('string','Go','callback','browseEventsRnd gotoEvent',...
...
% controls for showing the tracked object coordinates
   uicontrol('style','checkbox','string','Track','value',1,...
       ...
       'callback','browseEventsRnd track')
   
% controls to tag and remove ministacks
   uicontrol('style','text','position',[20,230,70,15],...
       'string','removed file')
   uicontrol('callback','browseEventsRnd loadRemoved','string','Load',...
       'position',[20,215,70,15],...
       'TooltipString',' Loads a removed event file ')
   uicontrol('callback','browseEventsRnd saveRemoved','string','Save',...
       'position',[20,200,70,15],...
       'TooltipString',' Saves the removed event file ')   
   uicontrol('style','checkbox','callback','browseEventsRnd removeEvent',...
...
% controls to write comments
   ...
   uicontrol('tag','commentEvent','callback','browseEventsRnd addComment',...
       'string','Add','position',[65,105,25,15],'userdata',comments)
   uicontrol('style','edit','tag','comment','position',[5,88,100,17],...
      'horizontalalignment','left')
   uicontrol('callback','browseEventsRnd loadComments','string','Load',...
       'position',[20,73,35,15])
   uicontrol('callback','browseEventsRnd saveComments','string','Save',...
       'position',[55,73,35,15])
   
% misc. controls   
   Fluo = cell(1,4);
   uicontrol('callback','browseEventsRnd writeExcel','string','WriteXLS',...
       'position',[20,55,70,15],'tag','Fluo','userdata',Fluo,...
       'tooltipstring',' Write failed/passed events in summary excel file')
   uicontrol('callback','browseEventsRnd removeTRC','string','RemoveTRC',...
       'position',[20,40,70,15],'tag','TRC','userdata',sTRC,...
       'tooltipstring',' Removes tagged events from trc file permanently')
%%%
   
   set(gcf,'UserData',M,'keypressfcn','browseEventsRnd keyBrowse','doublebuffer','on',...
      'colormap',map,'tag',[stkd,stk])
      %'closerequestfcn','browseEventsRnd saveRemovedandClose')

   ministack(sRndEv(1,2)) %The actual number of the first randomized event
   mzoom on
   %pixvalm
   scale
   goto
...
M = get(gcf,'userdata');
width = round(size(M,1)./4); % 3 in browseEvents2
M7 = M(1:width,:,:);
...
h_rem = findobj(children,'tag','removeCause');
sRndEv = get(findobj(children,'tag','sRnd'),'userdata');
sR = sRndEv(:,2)==numEv;
numRnd = find(sR);
numRnd = numRnd(1);
if events(1,1) == 0
...
uicontrol('style','slider',...
      'callback','browseEventsRnd scaleRe',...
...
uicontrol('callback','browseEventsRnd goto','tag','frame#',...
      'style','slider',...
...
uicontrol('style','edit','tag','EvNumber','string',num2str(numRnd),...
...
evCrd = ...
['  Event ',num2str(numRnd),' Start at X ',num2str(xGreen),', Y ',num2str(yGreen),', fr ',num2str(frame)];
h = title(['Cell ',cellNum(c:c2),evCrd,'  Frame # = ',num2str(-param(2))],...
    'interpreter','none');
set(h,'userdata',['Cell ',cellNum(c:c2),evCrd])
axis image
pixvalm
browseEventsRnd scaleGr
...
elseif get(gcf,'currentcharacter') == 'c'
    children = get(gcf,'children');
    Mini = get(findobj(children,'tag','axisGr'),'userdata');
    frame = round(get(findobj(children,'tag','frame#'),'value'));
    if frame>1
...
elseif get(gcf,'currentcharacter') == 'n'
    browseEventsRnd nextEvent
elseif get(gcf,'currentcharacter') == 'p'
    browseEventsRnd previousEvent
end
...
children = get(gcf,'children');
events = get(findobj(children,'tag','trackEvents'),'userdata');
numRnd = str2num(get(findobj(children,'tag','EvNumber'),'string'));
if isempty(numRnd),return,end
sRndEv = get(findobj(children,'tag','sRnd'),'userdata');
numRnd = max(numRnd,1);
numRnd = min(numRnd,size(sRndEv,1));
numEv = sRndEv(numRnd,2);
ministack(numEv)
%% Tracking functions
function track
children = get(gcf,'children');
...
%%
function loadRemoved %Changed in browseEventsRnd
...
function loadRnd
button = questdlg('Do you want to load a randomization file?',...
    'Load randomization');
if strcmp(button,'Yes')
    [f,p] = uigetfile('*.txt','File with randomization number of events');
    if ~f,return,end
    sRndEv = load([p,f]);
    children = get(gcf,'children');
    set(findobj(children,'tag','sRnd'),'userdata',sRndEv) 
    ministack(sRndEv(1,2)) 
    %numEv = get(findobj(children,'tag','EvNumber'),'userdata');
    %set(findobj(children,'tag','comment'),'string',comments{numEv})
end
...
[fle,pth] = uiputfile([cellName(c:d),'_rand.txt'],...
'Where to put the comments file');
if ischar(fle)&&ischar(pth)
   dlmwrite([pth,fle],sRndEv,'\t')
end
%%
function writeExcel
...
pause(0.1)
    [f,p] = uiputfile([cellNumber,'_browsed.xlsx'],...
...
%%% Draws a figure of the graphs
    ...
    [ffluo,pfluo] = uiputfile([cellNumber,'_fluo.fig'],'Save fluo data');
    if ischar(ffluo)&& ischar(pfluo)
        saveas(h_xls,[pfluo,ffluo])
    end
