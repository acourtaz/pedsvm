...
for i=1:(size(events,1)-3)
    if events(i,1) == 0
        ...
        if color > 0
            ...
            fraction = p/j;
            val(5)=fraction;
        else removed = cat(1,removed,val(1));
        end
...
u = strfind(f,'_');
u = u(2);
[fle,p] = uiputfile([f(1:u),'connect.txt']...
      ,'Where to put the file connecting events with parent structures');
if ischar(fle)&ischar(p)
   dlmwrite([p,fle],output,'\t')
end

[fre,p] = uiputfile([f(1:u),'unc_removed.txt']...
      ,'Where to put the file with unconnected events');
if ischar(fre)&ischar(p)
   dlmwrite([p,fre],removed,'\t')
end