function [PkFound,SeqLen,Xsize,Ysize] = seqfind_old (file,Option,Image,nX,nY)
%-----------------------------------------------------------------------------
% SEQFIND.M
...
if length(result)>0
  ...
  if strcmp(MASCHINE(1:2),'AT')
    DoIt  = ['save pk',filesep,file,' result -ascii']
  elseif strcmp(MASCHINE(1:2),'PC')
    DoIt  = ['save pk',filesep,file,'.pk result -ascii']
  else
