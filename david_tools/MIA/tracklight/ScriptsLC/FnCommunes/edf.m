...
%   if reduc == 1, reduction de la taille de l'image voir speread et dataread ligne 70 pour la valeur de la r�uction
% 050600 V1.0 by GAB
...
%   path=find(filespec=='/' | filespec=='\'); % any path
   path=find(filespec==filesep); % any path %% ???????
   if path>0
...
%	   path=find(actfile=='/' | actfile=='\'); % any path
	   path=find(actfile==filesep); % any path
