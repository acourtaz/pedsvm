...
for i=1:Maxtrace
    ...
    sizex=size(temp(:,2));
    maxfit=min(sizex(1),5); %nombre de points fitt�, ici au max 5
    ...
    if saveopt==1
        ...
        save(['msd',filesep,'molecules',filesep,filename,'.molecule',num2str(i),'.dat'],'temp','-ascii'); 
        %sauve l'ensemble des fits
        save(['msd',filesep,'fits',filesep,filename,'.fit.dat'],'lesfits','-ascii');
        save(['msd',filesep,'fits',filesep,filename,'.fit.dat'],'touslesfits','-ascii');
    end
...
lesfits=load(['msd',filesep,'fits',filesep,filename,'.fit.dat']);
