...
if ischar(file) % filename has been given
	...
	if strcmp(MASCHINE(1:2),'AT')
	  DoItPk  = ['load pk',filesep,file];
	  DoItTrc = ['load trc',filesep,file];
	elseif strcmp(MASCHINE(1:2),'PC')
	  DoItPk  = ['load pk',filesep,file,'.pk'];
	  DoItTrc = ['load trc',filesep,file,'.trc'];
	else
	  DoItPk  = ['load ',file,'.pk'];
...
if mode==1 
  if strcmp(MASCHINE(1:2),'AT')
    DoIt  = ['save ind',filesep,file,' TraceIndex /ascii'];
  elseif strcmp(MASCHINE(1:2),'PC')
    DoIt  = ['save ind',filesep,file,'.ind TraceIndex /ascii'];
  else
