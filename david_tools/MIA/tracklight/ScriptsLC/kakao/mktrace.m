...
%clear input-data
Peaks = clearpk (Peaks,Conf,3,Opts(18)); %enlève les doubles peaks et les peaks de largeur>opts(18)(ou 4 par défaut) pixels et <1 pixels
size(Peaks)
No_Pk = length(Peaks);
...
for iImage=Peaks(1,1)+1:max(Peaks(:,1))
  ...
  Im2  = Peaks(ind2,2:3);
  cm   = track (Im1,Im2,D,Xmax,Ymax,TOpts,OMode);
...
% indb1 = cm(find(cm(:,1)<=length(ind1)&cm(:,2)>length(ind2),1);
  ...
  %recovering peaks
  cr = track (Im0(indb0,:),Im2(indr,:),4*D,Xmax,Ymax,TOpts,OMode);
