...
% mol�ules et les synapses trait�s par MIA �partir des fichiers pk. Les
% cutoffs filtrent les pk donn� par edfPCS comme d'hab.
% r�ultat: les distances en pixels (DisttoCentr et DisttoMax) et les centroides (CentredesSyn) et maximas locaux(maxdesSyn) des
% synapses
...
pkdata = pkdata(pkind,:);
disp(['Reste ',num2str(size(pkdata,1)),' pics apr� cutoffs.']);
...
%%%%%%%%% x et y sont invers� dans synapses par rapport aux r�ultats de
%%%%%%%%% edf
Xdim=p(1)
Ydim=p(2)/p(4)
clear d, p, t, c;
...
%les densit� surfacique (u.A.): je divise par distance
for i=1:DistanceMax
