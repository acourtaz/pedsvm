...
st = {d.name};
if isempty(st)==1
   msgbox(['No files!!'],'Select trace files','error')
   return
end
[listafiles,v] = listdlg('PromptString','Select trace files:','SelectionMode','multiple','ListString',st);
if v==0
   return
end
...
% CMT - 22/05/2008 do away with the dialog box, just get a background image
% not ready yet !
% [fn,pn,fi]=uigetfile({'*.spe;*.stk;*.tif'},'Select background image');
% if(~ischar(fn) && fn==0)
%   msgbox('Cannot continue without a background file','Error','error');
%   return;
% end;
% dicfile=[pn,fn];

% dialog box 
   prompt = {'(leave empty or press CANCEL to open a user-defined file)'};
 num_lines= 1;
dlg_title = 'Enter identifier for background file';
def = {'dic'}; % default values
answer  = inputdlg(prompt,dlg_title,num_lines,def,'on');
% whos answer
% isempty(deblank(answer{1}))
% strcmp(answer,'')
exit=size(answer);
dicfile=''; % by CMT 27/04/2008 -- see below
% passID=false;
if (isempty(deblank(answer)) || isempty(deblank(answer{1})) || exit(1) == 0)
  % by CMT 27/04/2008: if no background identifier is returned, then select a
  % background image file by means of an uigetfile dialog
%   passID=false;
  [fn,pn,fi]=uigetfile({'*.spe;*.stk;*.tif'},'Select background image');
  if(~ischar(fn) && fn==0)
    msgbox('Cannot continue without a background file','Error','error');
...
% by CMT 27/04/2008 -- see above
if(isempty(dicfile) || cont > startcont)
  ...
  control=1;
  if(~isempty(strfind(dicfile,'.spe')))
    stktrue=0;
  elseif(~isempty(strfind(dicfile,'.tif')) || (~isempty(strfind(dicfile,'.spe'))))
    stktrue=1;
  end;
end;
...
if control==1
  ...
  if stktrue==0
          ...
          if(ver<=14)
            imshow(datamatrix,'notruesize'); % by CMT 27/04/2008
          else
          ...
          if stktrue==3
            if(ver<=14)
              ...
              imshow(filename,'InitialMagnification','fit');
            end;
              hold on;
          else
