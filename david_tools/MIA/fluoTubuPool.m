function fluoTubuPool(f,stkG,stkR,fmask,fback)
...
%For use with pooled data, (organized in 1 folder per cell)
...
%if ~stkG,return,end
mG = stkread(stkG,pbroot);
pause(0.5)
maskcell=dlmread(fmask,'\t');
...
%if ischar(fle) && ischar(p)
   ...
   xlswrite(fle,EvCell,sheetEv);
   sheetG = [stkG(1:end-4),' Green'];
   xlswrite(fle,output,sheetG)
   if stkR
       sheetR = [stkR(1:end-4),' red'];
       xlswrite(fle,outputR,sheetR)
   end
%end
