...
%Estimate of red fluorescence level for normalization
    ...
    fluoback=sum(sum(double(mR(regionback(1,2):(regionback(1,2)+regionback(1,4)), regionback(1,1):(regionback(1,1)+regionback(1,3))))))/(regionback(1,3)*regionback(1,4));
    Redmoy = maskcell.*double(mR(:,:,1));
    Redmoy = ((sum(sum(Redmoy)))/airepix)-fluoback;
...
if stkR
    ...
    parcellR(1,9)={'Average red'};
    parcellR(2,9)=num2cell(Redmoy);
    outputR = cat(1,parcellR,outputR);
end

[fle,p] = uiputfile('.xlsx',...
