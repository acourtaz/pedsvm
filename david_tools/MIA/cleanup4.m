...
slopeMax = 1;
thClst = 0.2;
minFrame = 5; % Number of frames before the event
...
reject = zeros(lastEvent,15);
accept = zeros(lastEvent,8);
 ...
 %Condition: fluo of object above background
                if SN > sigNoise
                    ...
                    output(k:k+length-1,:) = events(start:start+length-1,:);
                        ...
                        accept(a,5) = length-1;% +1 at line 145
                        accept(a,6) = slope;
                        accept(a,7) = SN;
                        accept(a,8) = coClst;
                        a = a+1;
...
tableSum(15,1:8) = {'trk id','fr','x','y','lifetime','slope','S/N','cluster'};
...
if ~isempty(accept)
    tableSum(16:15+size(accept,1),1:8) = num2cell(accept);
end
if ~isempty(reject)
...
[fexcel,p] = uiputfile([cellNum,'_candidates(',num2str(minFrame),').xlsx'],...
...
if ~isempty(iSN) && ~isempty(accept)
    ...
    axis([-1 1 0 1])
    title('slope')
    text(-2,1.1,[num2str(a-1),' candidates']) % Mo added the -1 because a is 
