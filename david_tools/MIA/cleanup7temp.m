...
sigNoise = 500; %Signal/Noise ratio estimated on TfR5 movie
...
prompt = {'Circle radius','Annulus outer radius',...
    ...
    'Signal/Noise ratio (absolute for temp)',...
...
double(movi(xy1(2)-rAnn:xy1(2)+rAnn,xy1(1)-rAnn:xy1(1)+rAnn,frame-5:frame-1));
            ...
            %stdev = sqrt(sum((average-avbefore).^2))./2; %From cleanup5
            %if stdev ~= 0
...
if ~isempty(newStart)
    for i = nad
        ...
        if u
            ...
            evNum = round(output(sta,5));
            evTr = (output(1:sta-1,5)==evNum); 
...
%the same parent event evNum. There should be only one in the upper part.
            ...
            if v
                ...
                else %if added event is less than 2 frames after parent, parent is removed 
                    if stp+lp < size(output,1)
                        output = output([1:stp-1 stp+lp:end],:);
                    else
                        output = output(1:stp-1,:);
                    end
                    pTrack = (accept(:,1)==evNum);
                    [w,s] = max(pTrack);
...
if ~isempty(iSN) && ~isempty(accept)
    ...
    axis([-1 1 0 1])
    title('slope')
    text(-2,1.1,[num2str(a-1),' candidates'])
