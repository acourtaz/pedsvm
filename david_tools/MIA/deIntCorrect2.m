function deIntCorrect

%written by DP. Last update 27/09/2008
%Deinterlaces the pH5 and pH7 frames and 
%correct the red channel for bleed-through

[f,p] = uigetfile('*.xls','Data file with fluo measures');
if ~f,return,end
...
for i=1:size(sheets,2)
    ...
    if ~isempty(isdata5)
        if isempty(strfind(sheets{i}(5:5+isdata5),'TfR'))
            red5 = i;
            prot = sheets{i}(6:8);
        else
            gre5 = i;
    ...
    elseif ~isempty(isdata7)
        if isempty(strfind(sheets{i}(5:5+isdata7),'TfR'))
            red7 = i;
        else
            gre7 = i;
        end
...
[dataRed5,textRed5] = xlsread([p,f],sheets{red5});
[dataGre5,textGre5] = xlsread([p,f],sheets{gre5});
[dataRed7,textRed7] = xlsread([p,f],sheets{red7});
[dataGre7,textGre7] = xlsread([p,f],sheets{gre7});
...
if isempty(bleed)
    ...
    for i = 1:51
        ...
        SSquares(i) = sum(sqDiff);
    end
    [u,miniBT_X]= min(SSquares);
    bleed = BT(miniBT_X);
    figure('name',[cellName,' bleedthrough'])
    plot(BT,SSquares,'o')
...
if ischar(fle) && ischar(p)
   ...
   newsheet = [sheets{red5}(1:end-6),' deint'];
...
%%%%% Figure of signal/noise histogram
    ...
    figure('name',[cellName,' threshold'])
...
%%%%% Figure of interval histogram
    figure('name',[cellName,' intervals'])
...
titColumns = {'event #','noise1','noise2','noise','time 1st','1st max val','time 2nd','2nd max val','interval between peaks','time 3rd','3rd max val'};
    ...
    if ischar(fle) && ischar(p)
        warning off MATLAB:xlswrite:AddSheet
        histsheet = [cellName,' histo'];
        xlswrite([p,fle],results,histsheet)
        newfluosheet = [cellName,' newfluo'];
