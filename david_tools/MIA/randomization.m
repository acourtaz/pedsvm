function randomization(nTrials)

%written by David Perrais, 25/10/08;updates...13/01/2015
...
pause(0.1)
[stkGre5,stkdGre5] = uigetfile('*.stk','Stack of receptor (TfR) at pH 5');
...
if ~isempty(strfind(stkGre5,'TfR'))
    rcp = 'TfR';
elseif ~isempty(strfind(stkGre5,'B2R'))
    rcp = 'B2R';
end
                
pause(0.1)
[stkGre7,stkdGre7] = uigetfile('*.stk','Stack of receptor (TfR) at pH 7');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
edge = rAnn+7; %minimum distance from edge % should be +5
...
Xmax = ceil(max(events(:,3)))-2; % -2 to remove (more drastic shift limitation)
Ymax = ceil(max(events(:,4)))-2; % -2 to remove (more drastic shift limitation)
erMask = mask;
%se = strel('disk',edge); 
...
pts = 2*(before+length);
%Recalculates the real data for figure display
XYr = zeros(size(XYshift(:,:,1)));
reGre5 = calcFluoR(mGre5,events,evNum,XYr,param,ncoeff,x,y,1);
reGre7 = calcFluoR(mGre7,events,evNum,XYr,param,ncoeff,x,y,1);
reRed5 = calcFluoR(mRed5,events,evNum,XYr,param,coeff,x,y,1);
reRed7 = calcFluoR(mRed7,events,evNum,XYr,param,coeff,x,y,1);
if first_pH == 5
    reRed(1:2:pts) = reRed5 - bleed.*reGre5;
    reRed(2:2:pts) = reRed7 - bleed.*reGre7;
elseif first_pH == 7
    reRed(1:2:pts) = reRed7 - bleed.*reGre7;
    reRed(2:2:pts) = reRed5 - bleed.*reGre5;
end

for j = 1:nTrials
    randGre5 = calcFluoR(mGre5,events,evNum,XYshift(:,:,j),param,ncoeff,x,y,0);
    trialsGre5 = cat(1,trialsGre5,randGre5);
    randGre7 = calcFluoR(mGre7,events,evNum,XYshift(:,:,j),param,ncoeff,x,y,0);
    trialsGre7 = cat(1,trialsGre7,randGre7);
    randRed5 = calcFluoR(mRed5,events,evNum,XYshift(:,:,j),param,coeff,x,y,0);
    randRed7 = calcFluoR(mRed7,events,evNum,XYshift(:,:,j),param,coeff,x,y,0);
...
subplot(1,3,1)
plot(timeGre7',reGre7','-o','color',[0 0.7 0],'markerfacecolor',[0 0.7 0])
hold on
plot(timeGre7',envGre7(:,1),'k','linewidth',2)
plot(timeGre7',envGre7(:,2),'k')
plot(timeGre7',envGre7(:,3),'k','linewidth',2)
line([0 0],ylim)
hold off

subplot(1,3,2)
plot(timeGre5',reGre5','-og','markerfacecolor','g')
hold on
plot(timeGre5',envGre5(:,1),'k','linewidth',2)
plot(timeGre5',envGre5(:,2),'k')
plot(timeGre5',envGre5(:,3),'k','linewidth',2)
line([0 0],ylim)
hold off

subplot(1,3,3)
plot(timeRed',reRed','-or','markerfacecolor','r')
hold on
plot(timeRed',envRed(:,1),'k','linewidth',2)
plot(timeRed',envRed(:,2),'k')
plot(timeRed',envRed(:,3),'k','linewidth',2)
line([0 0],ylim)
hold off
...
headerEnv{6,6} = [rcp,'_7'];
headerEnv{6,11} = [rcp,'_5'];
cellEnv = [headerEnv;cellEnv];
sheets = {[cellN,' env'],[cellName,'_rnd'],[cellN,'_',rcp,'7 rnd'],...
    [cellN,'_',rcp,'5 rnd']};

[frnd,prnd] = uiputfile([cellN,'_rand(',num2str(nTrials),').xlsx']...
...
%param = [numberEvents,before,length,rCircle,rAnn,lowpercent,highpercent]
function avFluoR = calcFluoR(movi,events,evNum,XYs,param,coeff,x,y,a)
...
for i=firstEvent:lastEvent
    ...
    if u
        ...
        k = k(1);
        if ~(XYs(k,1)==0 && XYs(k,2)==0)||a %Will ignore the events were no shift 
