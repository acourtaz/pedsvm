...
for i=1:size(sheets,2)
    isTfR5 = strfind(sheets{i},'B2R5');
    isTfR7 = strfind(sheets{i},'B2R7');
...
if numTerm == 0
    ...
    avTerm7 = mean(sortData7(1+numOutLow:numOutLow+numTerm,2:end));
    semTerm7 = std(sortData7(1+numOutLow:numOutLow+numTerm,2:end))./sqrt(numTerm);
    avTerm5 = mean(sortData5(1+numOutLow:numOutLow+numTerm,2:end));
    semTerm5 = std(sortData5(1+numOutLow:numOutLow+numTerm,2:end))./sqrt(numTerm);
    avTermRed = mean(sortDataRed(1+numOutLow:numOutLow+numTerm,2:end));
    semTermRed = std(sortDataRed(1+numOutLow:numOutLow+numTerm,2:end))./sqrt(numTerm);
end
...
if numNTerm == 0
    ...
    avNTerm7 = mean(sortData7(end-numOutUp-numNTerm+1:end-numOutUp,2:end));
    semNTerm7 = std(sortData7(end-numOutUp-numNTerm+1:end-numOutUp,2:end))./sqrt(numNTerm);
    avNTerm5 = mean(sortData5(end-numOutUp-numNTerm+1:end-numOutUp,2:end));
    semNTerm5 = std(sortData5(end-numOutUp-numNTerm+1:end-numOutUp,2:end))./sqrt(numNTerm);
    avNTermRed = mean(sortDataRed(end-numOutUp-numNTerm+1:end-numOutUp,2:end));
    semNTermRed = std(sortDataRed(end-numOutUp-numNTerm+1:end-numOutUp,2:end))./sqrt(numNTerm);
end
...
xlabel('time (s)')
ylabel('average fluo')
title(['cell #  ',cellNum,' B2R7 tnt'])
hold off

[f7,p7] = uiputfile([cellNum,' tnt_B2R7'],'save figure');
if ischar(f7)&& ischar(p7)
    saveas(gcf,[p7,f7])
end

figure('name',[cellNum,' B2R5 tnt'])
...
xlabel('time (s)')
ylabel('average fluo')
title(['cell #  ',cellNum,' B2R5 tnt'])
hold off

[f5,p5] = uiputfile([cellNum,' tnt_B2R5'],'save figure');
if ischar(f5)&& ischar(p5)
    saveas(gcf,[p5,f5])
end

ratio7cell = num2cell(ratio7);
ratio7cell = cat(1,{'ratio B2R7'},ratio7cell);
...
%B2R7 data
cellAv{5,12} = 'B2R7 total';
cellAv{5,15} = 'B2R7 non terminal';
cellAv{5,18} = 'B2R7 terminal';
cellAv{6,11} = 'time';
...
%B2R5 data
cellAv{5,22} = 'B2R5 total';
cellAv{5,25} = 'B2R5 non terminal';
cellAv{5,28} = 'B2R5 terminal';
cellAv{6,21} = 'time';
...
disp(['terminal (green) : ',num2str(cellAv{7,20}/(cellAv{7,20}+cellAv{7,17})*100),'%'])
