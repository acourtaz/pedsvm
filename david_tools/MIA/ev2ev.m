function varargout = ev2ev(varargin)
...
for i = 1:size(ev1,1)
    startFr = ev1(i,2);    
    j = find(ev2(:,2)== startFr); % indices for which an event appering in 
                                  % ev1 appears at the same frame in ev2
    if ~isempty(j)        
...
[fevSt,pevSt] = uiputfile(['[',fEv1,']_[',fEv2,']_compare.trc'],'Save file');
