...
for i=1:size(sheets,2)
    ...
    if ~isempty(isdata5)
        if isempty(strfind(sheets{i}(5:5+isdata5),'B2R'))
            red5 = i;
    ...
    elseif ~isempty(isdata7)
        if isempty(strfind(sheets{i}(5:5+isdata7),'B2R'))
            red7 = i;
        else
            gre7 = i;
        end
...
[fbleed,pbleed] = uiputfile([f(1:c),'_bleedthrough',num2str(bleed*1000)],...
    'Save Bleedthrough coefficient');
if ischar(fbleed) && ischar(pbleed)
    saveas(gcf,[pbleed,fbleed]);
end
...
averagePlot = mean(newdata(8:end,4:end),1);
...
[fdeInt,pdeInt] = uiputfile([cellNum,'_',prot], 'Save Corrected Red quantification');
saveas(gcf,[pdeInt,fdeInt]);
