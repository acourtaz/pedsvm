...
if isempty(varargin)
    ...
    events = dlmread([p,f],'\t');
    fmax = [];
    [fmax,pmax] = uigetfile('*.txt;*.trc',...
        'File with event Maximums (Press Cancel if it is not needed)');
    if ~fmax
        eventsMx = [];
    else
        eventsMx = dlmread([pmax,fmax],'\t');
    end
...
%Verifies that the object trajectory is in the image for red quantification        
    ...
    if sum(sum(edge_xy))==0
        ...
        j=0; 
%calculates avFluo for the tracked event
...
sum(sortback(miniPix-nPix+nLoPix+1:miniPix-nPix+nHiPix))/(nHiPix-nLoPix);
            j=j+1;
            valTr(j) = sum(sum(im.*circle))/sum(sum(circle)) - background;
        end
%finds the maximum fluorescence in the tracked frames
        valTr = valTr(1:j);
        val(2) = j;
        if isempty(eventsMx) %eventsMx: the imported Max file
            [valMax,frMax] = max(valTr);
            evMax = cat(1,evMax,events(start+frMax-1,:));
        else
            maxTrack = (eventsMx(:,1)==i);
            [u_Max,s_Max] = max(maxTrack);
            if u_Max
                l_max = events(:,1)==i & events(:,2)==eventsMx(s_Max,2);
                [valMax,fr_Max] = max(l_max);
                frMax = fr_Max-start+1;
                evMax = cat(1,evMax,events(frMax,:));
            end
        end
        
%calculates fluo outside tracked frames if necessary and puts into output        
        if frMax+after <= j
            val(before+5:length+3) = valTr(frMax+1:frMax+after);
        else
            val(before+5:j-frMax+before+4) = valTr(frMax+1:j);
            v = min([frame+frMax+after-1,size(movi,3)]);
            v = v - frame - j+1; %The maximal length of remaining frames
%Calculations of center, distance, already performed in the while loop
            if v
                circle = circle(:,:,ones(1,v));
                annulus = annulus(:,:,ones(1,v));
                im = ...
                    double(movi(iy0-rAnn:iy0+rAnn,ix0-rAnn:ix0+rAnn,frame+j:frame+j+v-1));
                nPix = squeeze(round(sum(sum(annulus,1))))';
                nLoPix = round(nPix*lowpercent);
                nHiPix = round(nPix*highpercent);
                back = im.*annulus;
                sortback = sort(reshape(back,miniPix,v));
                background = ...
sum(sortback(miniPix-nPix+nLoPix+1:miniPix-nPix+nHiPix,:))./(nHiPix-nLoPix);
                values = sum(sum(im.*circle))/sum(sum(circle(:,:,1)));
                val(j-frMax+before+5:j-frMax+before+5+v-1) = squeeze(values)' - background;
            end
        end
        ...
        if frMax > before
            ...
            w = min([frame-1,before-frMax+1]);%number of frames to be calculated
            if w
                ...
                distance = sqrt((x-rAnn-dx0-1).^2 + (y-rAnn-dy0-1).^2);
                circle = distance<rCircle;
                circle = circle(:,:,ones(1,w+1)); 
 %recalculates first tracked frame; to make sure w+1 > 1 
                annulus = (distance>=rCircle)&(distance<rAnn);
                annulus = annulus(:,:,ones(1,w+1));
                im = ...
double(movi(iy0-rAnn:iy0+rAnn,ix0-rAnn:ix0+rAnn,frame-w:frame));
                nPix = squeeze(round(sum(sum(annulus,1))))';
                nLoPix = round(nPix*lowpercent);
                nHiPix = round(nPix*highpercent);
                back = im.*annulus;
                sortback = sort(reshape(back,miniPix,w+1));
                background = ...
        sum(sortback(miniPix-nPix+nLoPix+1:miniPix-nPix+nHiPix,:))./(nHiPix-nLoPix);
                values = sum(sum(im.*circle))/sum(sum(circle(:,:,1)));
                val(4:w+4) = squeeze(values)' - background;
            end
        end
...
if isempty(cp)
    pname = ' ';
else pname = stk(cp(end)-3:cp(end)-1);
end

hf = figure('name',stk(1:end-4));
if ~coFile
...
if isempty(varargin)
[fquant,pquant] = uiputfile([f(1:c-1),'_',pname,'.fig'],'Save quantification');
if ischar(fquant) && ischar(pquant)
    saveas(hf,[pquant,fquant])
end
else
