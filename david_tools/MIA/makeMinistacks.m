function makeMinistacks

%Written by DP 6/09/05 - updated 4/02/06 - 21/03/07%
...
for i=counter
    ...
    if u
        ...
        xa = round(interPoly(events(start,3),events(start,4),coeff))+sTRC;
        ya = round(interPolx(events(start,3),events(start,4),coeff))+sTRC;
