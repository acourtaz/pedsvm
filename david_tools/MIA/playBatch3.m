function playBatch3(action)

% written by DP
% last updated 01/09/06

if nargin == 0
   [stkl,stkld] = uigetfile('*.stk','Choose a (mini)Stack (Left image)');
   if ~stkl,return,end
   [stkr,stkrd] = uigetfile('*.stk','Choose a (mini)Stack (Right image)');
   if ~stkr,return,end
   frame = 1;
   %makes sure the two selected ministacks bear the same number
   la = strfind(stkl,'_');
   la = la(size(la,2))+1;
   lb = strfind(stkl,'.');
   lb = lb(size(lb,2))-1;
   ...
   rb = strfind(stkr,'.');
   rb = rb(size(rb,2))-1;
   stkr = [stkr(1:ra-1),numEv,stkr(rb+1,end)];
   
   Mleft = stkread(stkl,stkld);
   if exist([stkrd,stkr])==0
       Mright = zeros(size(Mleft));
   else
       Mright = stkread(stkr,stkrd);
   end
   ...
   uicontrol('string','Stop','callback','playBatch3 goto',...
       'position',[20,30,45,15])
   uicontrol('callback','playBatch3 fullforward','string',...
      'Play -->','position',[110,30,45,15])
   uicontrol('callback','playBatch3 fullbackward','string',...
  ...
  %controls for the scale
   ...
   uicontrol('callback','playBatch3 goto',...
      'style','slider',...
   ...
   uicontrol('callback','playBatch3 previousEvent','string','PREVIOUS',...
       'position',[20,180,70,15])
   uicontrol('callback','playBatch3 nextEvent','string','NEXT',...
       'position',[20,165,70,15])
   uicontrol('style','text','string','#','position',[20,150,10,15])
   
   uicontrol('style','edit','tag','EvNumber','string',stkl(a:b),...
       'position',[30,150,40,15])
   uicontrol('string','Go','callback','playBatch3 gotoEvent',...
   ...
   set(gcf,'UserData',M,'keypressfcn','playBatch2 key',...
      'doublebuffer','on',...
      'colormap',map,'tag',[stkld,stkl],...
      'closerequestfcn','playBatch3 saveRemovedandClose')
...
uicontrol('style','slider',...
      ...
      'value',high,...
      'position',[240,30,180,15],'tag','scalehigh')
uicontrol('callback','playBatch3 goto','style','slider',...
      'position',[55,15,100,15],...
      'max',size(M,3),...
      'min',1,...
      'value',1,...
...
uicontrol('style','edit','tag','EvNumber','string',num2str(numEv),...
      'position',[30,150,40,15])
set(gcf,'UserData',M,'keypressfcn','playBatch3 key','doublebuffer','on'...
