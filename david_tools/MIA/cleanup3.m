...
%written by DP - last update 22/11/07
...
maxFrame = 20;
defaults = [rCircle,rAnn,Edge,sigNoise,slopeMax,thClst,minFrame,maxFrame];
...
[rCircle,rAnn,Edge,sigNoise,slopeMax,thClst,minFrame,maxFrame] = ...
numinputdlg(prompt,'Parameters for removing non qualified events',1,defaults);
pause(1)
params = [rCircle,rAnn,Edge,sigNoise,slopeMax,thClst,minFrame,maxFrame];
...
%(as an approaching object would do)
 ...
 %Condition: fluo of object above background
   ...
   %rejection: pre existing cluster too small
        ...
        reject(r,8) = 1; %rejection: event is too early
        r = r+1;
    end
    end
end
...
counts = [(r+a-2);sumReject(8:14)';(a-1)];
accept = accept(1:a-1,:);

cellHigh = max(size(accept,1),size(reject,1))+15;
tableSum = cell(cellHigh,22); %the cell array that will contain the xl sheet
tableSum(1:3,1) = {'exp type:';'analysis:';'date:'};
tableSum{3,2} = date;
tableSum(5:12,1) = {'rCircle';'rAnnulus';'Edge';'S/N';'(+)slope';'cluster';'fr_bef';'fr_aft'};
tableSum(5:12,2) = num2cell(params');
...
tableSum{14,9} = 'failed events';
tableSum(15,1:7) = {'trk id','fr','x','y','slope','S/N','cluster'};
tableSum(15,9:15) = {'trk id','fr','x','y','slope','S/N','cluster'};
tableSum(15,16:22) = {'frbef','fraft','edges','(+)slope','S/N','cluster','merge'};
tableSum(16:15+size(accept,1),1:7) = num2cell(accept);
tableSum(16:15+size(reject,1),9:22) = num2cell(reject);
...
[fexcel,p] = uiputfile([cellNum,'_data.xlsx'],...
...
if ~isempty(iSN)&~isempty(accept)
    ...
    slopeAccept = sort(accept(:,5));
    slopeGraph = sort(reject(iSN,5));
    SNAccept = sort(accept(:,6));
    SNGraph = sort(reject(iSN,6));
    clusterAccept = sort(accept(:,7));
    clusterGraph = sort(reject(iSN,7));
    totR = size(iSN,1);
    fractionR = 1/totR:1/totR:1;
    fractionA = 1/naccept:1/naccept:1;
    scr = get(0,'ScreenSize');
    figure('name','Thresholds cleanup',...
