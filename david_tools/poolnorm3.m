%Written by Damien Jullié, last modification on 30/04/12 exo speed
     ...
     %sort sheets
        for j=1:size(sheets,2)
           ...
           %average for the ten first green events 
            ...
            areaevent=sum(dataGreen(10:end,3))/size(m,1);

            length=sum(dataGreen(10:end,5))/size(m,1);
            ...
            dlmwrite([f(1:end-4),'P_annotate.txt'],AnnotP,'delimiter','\t');

            
            dlmwrite([f(1:end-4),'T_annotate.txt'],AnnotT,'delimiter','\t');
            ...
            %plot cummulative frequency (total)
            ...
            freq=(60*p(1))/dataGreen(1,13);
            
            %freq T linear regression
            frecumT=[1:size(AnnotT,1)]';
            pT=polyfit(AnnotT(:,2),frecumT,1);
            freqT=(60*pT(1))/dataGreen(1,13);

            
            %freq P linear regression
            frecumP=[1:size(AnnotP,1)]';
            pP=polyfit(AnnotP(:,2),frecumP,1);
            freqP=(60*pP(1))/dataGreen(1,13);

            
            
            poolav=[persprop,areaevent, length,dataGreen(2,13),dataRed(2,13),freq,size(AnnotT,1),size(AnnotP,1),freqT,freqP,moyenne,avred];
            ...
            %nouvel onglet event/avfluogreen
            
            normgreen = m/dataGreen(2,13);
            sheetNormGreen = [f,' NormGreen'];
            xlswrite('compilation.xlsx', normgreen, sheetNormGreen,'B2')
            normfinal=[normfinal;normgreen];
...
if isempty(finalpers)
    Avfinalpers=ones(1,41);
    ETfinalpers=ones(1,41);
else
...
if isempty(finaltrans)
    Avfinaltrans=ones(1,41);
    ETfinaltrans=ones(1,41);
else
...
figure
errorbar(-10:30,Avfinaltrans, ETfinaltrans,'-ob','MarkerEdgeColor','b')
hold on
errorbar(-10:30,Avfinalpers, ETfinalpers,'-oc','MarkerEdgeColor','c')
...
if isempty(finalpersr)
    Avfinalpersr=ones(1,41);
    ETfinalpersr=ones(1,41);
else  
...
if isempty(finaltransr)
    Avfinaltransr=ones(1,41);
    ETfinaltransr=ones(1,41);
else            
...
figure
errorbar(-10:30,Avfinaltransr, ETfinaltransr,'-ob','MarkerEdgeColor','b')
hold on
errorbar(-10:30,Avfinalpersr, ETfinalpersr,'-oc','MarkerEdgeColor','c')
...
legendpool={'cell','pers prop','area','length','average green','average red','frequency','Number T','Number P','FreqT','FreqP'};
...
xlswrite('compilation.xlsx', normfinal, 'NormAvGreen', 'B2')