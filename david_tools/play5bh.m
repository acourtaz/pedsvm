...
if nargin == 0
   ...
   uicontrol('style','edit','position',[60,360,40,40],'string','1000',...
       'tag','Thresh')
...
negThresh = 0; %threshold for "negative" values to reject events detected 
                  %because of movement. Put 0 or a very low value to make
                  %it inoperant
areaSize = 3; %Minimal size for an event to be valid
refrTime = 4; %Number of frame in which an event can't occur in the same place
defaults = [thresh,frbeg,frend,side,distMin,negThresh,areaSize,refrTime];
prompt = {'Threshold for event detection (from the main window)',...
    ...
    'Minimal size for an event to be valid',...
    'Number of frame in which an event can''t occur in the same place'};
[thresh,frbeg,frend,side,distMin,negThresh,areaSize,refrTime] = ...
...
dims = size(M);
data = cell(1,dims(3)); % one array per frame
segM = M>thresh;
segMneg = M<negThresh;
refrM = zeros(0,3); %Memory of the last occurence of events
for i=1+frbeg:dims(3)-frend %iterates over images of the movie
    lab = bwlabel(segM(:,:,i));
    prop = regionprops(lab,'basic');
    if ~isempty(prop)
        for j=1:size(prop,1) %for each potential event
            if prop(j).Area > areaSize
                XY = prop(j).Centroid;
                if (XY > 3+side)&(XY < [dims(1) dims(2)]-side-2) %#ok<AND2> %if not on the side of the frame
                   if i > 1
                       rejet = 0;
                       for u = 1:size(refrM,1)
                          if (i-refrM(u,3))<refrTime
                              dist = sqrt(sum(refrM(u,1:2)-XY).^2);
                              if dist < distMin 
                                 rejet = u;
                                 break;
                              end
                       ...
                       if rejet == 0 %validated event
                           rXY = round(XY);
                           darkMoll = segMneg(rXY(2)-side:rXY(2)+side,rXY(1)-side:rXY(1)+side,i);
...
%                       if ~isempty(data{i-1}) %detection of the same event on the n-1 image
%                           XYbef = data{i-1};
%                           dBef = zeros(size(XYbef,1),1);
%                           for u = 1:size(dBef,1)
%                               dBef = sqrt(sum(XY-XYbef(u,:)).^2); %distance between current candidate and old one
%                           end %there might be a better way than this loop...
%                           test = dBef > distMin;
%                       elseif ~isempty(data{i-2}) %detection of the same event on the n-1 image
%                           XYbef = data{i-2};
%                           dBef = zeros(size(XYbef,1),1);
%                           for u = 1:size(dBef,1)
%                               dBef = sqrt(sum(XY-XYbef(u,:)).^2);
%                           end %there might be a better way than this loop...
%                           test = dBef > distMin;
%                       else
%                           test = 1;
