...
param.mem = 5;                          % number of frames a particle can be 'lost'
...
res = track(pos,4,param);               % reassign tracks
...
b = 1;
for a = 1:ressize(1) - 1;
    if res(a,4) == res(a + 1,4) & (res(a + 1,3) - res(a,3)) > 1;
       ...
       temp(b + 1:b + gap - 1,1:4) = bridge(1:gap - 1,1:4);
       clear bridge
       b = b + gap;
    else
       temp(b,1:4) = res(a,1:4);
...
clear a b c dat dx dy f frs gap ids param p pos possize ressize xs ys res;
tempsize = size(temp);
warning on;
res = zeros(tempsize);
warning off;
