%function chopstk = chopstk(movi,trks,coeff,frbef,fraft,circ,ann,type);
...
[stk,stkd] = uigetfile('*.stk','Load parent stack');
if ~stk,return,end
...
elseif type == 3;
    ...
    for id = 1:400; %size(goodtrk,1);                                       % !!! temp cap on number of stacks
        ffr = goodtrk(id,2);
        lfr = goodtrk(id,3);
        tmptrk = trks(ffr:lfr,:);
        if lfr - ffr <= fraft + 1;
