function [varargout] = trkedit_mjt(varargin);

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @trkedit_mjt_OpeningFcn, ...
                   'gui_OutputFcn',  @trkedit_mjt_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before trkedit_mjt is made visible.
function trkedit_mjt_OpeningFcn(hObject, eventdata, handles, varargin);
% load stack of parent objects
parents = varargin{1};
handles.parents = parents;
...
parim = parents(:,:,filen);
tmpmin = min(min(parents(:,:,1)));
tmpmax = max(max(parents(:,:,1)));
parim = round(((parim - (1.1*tmpmin))/((tmpmax*1.1) - tmpmin))*64);
parim = ind2rgb(parim,grnmap);
image(parim);
...
% Choose default command line output for trkedit_mjt
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);

% --- Outputs from this function are returned to the command line.
function varargout = trkedit_mjt_OutputFcn(hObject, eventdata, handles) 
