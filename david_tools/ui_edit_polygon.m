...
%
% Interactive edit polylines (closed or open) and patches.
% If obj(handle_i) has already been made editable with
...
%       10-Nov-2005 Nice improvements and code cleaning
...
for (i = 1:length(varargin))            % Argument check
    ...
    % If the patch is already editable by ui_edit_polygon, disable the edit mode (if set on) 
...
switch key
    ...
    case char(98)                   % "b":      break line
        ...
        x = get(s.h_pol,'XData');    
        y = get(s.h_pol,'YData');
        x1 = x(1:s.vert_index);     x2 = x(1+s.vert_index:end);
        y1 = y(1:s.vert_index);     y2 = y(1+s.vert_index:end);
        s.is_closed = 0;            % Its not closed anymore
        
        if (length(x2) > 1)         % We don't want to let a stray point arround
            l2 = copyobj(s.h_pol,s.h_ax);
            set(l2,'XData',x2,'YData',y2);
            ui_edit_polygon(l2)
        end
        set([s.h_pol s.h_vert],'XData',x1,'YData',y1);
        %if (length(x1) == 1);   delete(s.h_pol);    return;     end       % Idem
        if (length(x1) == 1);   delete(s.h_pol);    end       % Idem
...
state = uisuspend(h_fig);           % Remember initial figure state
